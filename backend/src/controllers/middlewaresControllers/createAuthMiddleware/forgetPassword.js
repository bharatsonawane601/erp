...
const forgetPassword = async (req, res, { userModel }) => {
  ...
  const dbsquare_app_email = settings['dbsquare_app_email'];
  const dbsquare_base_url = settings['dbsquare_base_url'];

  const url = checkAndCorrectURL(dbsquare_base_url);
  ...
  await sendMail({
    ...
    name: user.name,
    link,
    subject: 'Reset your password | DB Square',
    dbsquare_app_email,
